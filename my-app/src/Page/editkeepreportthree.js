...
const Editkeepreportthree = () => {
  ...
  const renderFooter3 = (id) => {
    return (
      <div>
        ...
        <Button type="button" icon="pi pi-send" label='ส่ง' style={{ width: '7em', marginLeft: '.4em' }} onClick={() => updateproblem(id, editproblem, 1)} autoFocus />
      </div>
    );
  }
  ...
  const updatequartercharges = async (id, proid, editquartercharges, checked, statusreport) => {
    console.log('id', id)
    axios
    ...
    await updatereportthree(proid, statusreport)
    alert(`ต้องการจัดเก็บข้อมูลผลการใช้จ่ายใช่มั้ย?`)
  }

  const updatereportthree = async (id, statusreport) => {
    console.log('tt', id)
    axios
      .put(`http://localhost:3001/editkeepreport/updatereportthree/${id}`, {
        status_report3: statusreport
      })
  }
  ...
  const updatedetail = (id, editdetail, statusdetail) => {
    ...
    alert(`ต้องการจัดเก็บข้อมูลรายละเอียดความก้าวหน้าใช่มั้ย?`)
    showdetail()
  };
  ...
  const updateproblem = (id, editproblem, statusproblem) => {
    ...
    alert(`ต้องการจัดเก็บข้อมูลปัญหา/อุปสรรคใช่มั้ย?`)
    showproblem()
  };
          ...
          <div align="left">
            ...
            <Card>
              ...
              <div style={{ marginTop: '2em', marginLeft: '60em' }} >
                <Button type="button" icon="pi pi-download" label='จัดเก็บ' className="p-button-help" onClick={() => updatequartercharges(quartercharges[0].report_id, location.state.project_id, editquartercharges, checked, 0)} />
                <Button type="button" icon="pi pi-send" label='ส่ง' className="p-button-info" style={{ width: '7em', marginLeft: '.4em' }} onClick={() => updatequartercharges(quartercharges[0].report_id, location.state.project_id, editquartercharges, checked, 1)} />
              </div>
            </Card>
...
export default Editkeepreportthree