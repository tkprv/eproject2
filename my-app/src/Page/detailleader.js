...
const Detailleader = () => {
  ...
  const renderFooter1 = (name) => {
    return (
      <div>
        ...
        <Button label="อนุมัติ" icon="pi pi-check" className="p-button-success" onClick={() => confirmproject(location.state.project_id, 1)} />
      </div>
    );
  }
  ...
  const renderFooter2 = (name) => {
    return (
      <div>
        <Button label="ยกเลิก" icon="pi pi-times" className="p-button-danger" onClick={() => onHide(name)} />
        <Button label="ส่ง" icon="pi pi-check" className="p-button-success" style={{ width: '17%' }} onClick={() => noconfirmproject(location.state.project_id, 2)} />
      </div>
    );
  }
