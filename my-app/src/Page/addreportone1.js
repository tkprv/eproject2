...
const Addreportone = () => {
    ...
    const [addquarterchargesone, setAddquarterchargesone] = useState();
    ...
    useEffect(() => {
        showreportone(reportid)
        createdetailone(reportid)
        createproblemone(reportid)
    }, [reportid])
    ...
    const getquartercharges = () => {
        axios
          .get(`http://localhost:3001/quarterchargestone/${location.state.project_id}`, {})
          .then((res) => {
            console.log(res.data)
            setQuartercharges(res.data)
            setAddquarterchargesone(res.data[0].used)
    ...
    const createquarterchargesone = async (id) => {
        console.log('id', id)
        if (reportid === undefined || reportid === null) {
            axios
                .post(`http://localhost:3001/createquarterchargesone`, {
                    project_id: id,
                    quarter: 1,
                    used: addquarterchargesone,
            ...
            axios
                .put(`http://localhost:3001/updatequartercharges/${reportid}`, {
                    used: addquarterchargesone,
                    period_check: (checked === true) ? 1 : 0
                }).then((res) => {
    ...
    //   const updatereportone = async (id) => {
    //     console.log('tt', id)
    //     axios
    //       .put(`http://localhost:3001/updatereportone/${id}`, {
    //         report_one: 1
    //       })
    //       // await showreportone(reportid)
    //   }

    const showreportone = async (item) => {
        console.log('111', reportid)
        axios
            .get(`http://localhost:3001/showreport/${reportid}`, {})
            .then((res) => {
                // setReport(res.data[0].report_id)
            })
            .catch((error) => {
                console.log(error)
            });
        // await createdetailone()
        // console.log('44', report)
    };

    const createdetailone = async (id) => {
        console.log('id', id)
        axios
            .post(`http://localhost:3001/createdetailone`, {
                report_id: id,
                detail: adddetail
            }).then((res) => {
                showreportone()
    ...
    const createdetailone2 = (id) => {
        if (rowsData1.length !== 0 && rowsData1 !== null) {
            for (const value of rowsData1) {
                const detailone = value.rowsData1
                try {
                    axios.post('http://localhost:3001/createdetailone', {
                        report_id: id,
                        detail: value.detail,
                    })
                } catch (e) {
    ...
    const createproblemone = async (id) => {
        console.log('rr', reportid)
        console.log('id', id)
        axios
            .post(`http://localhost:3001/createproblemone`, {
                report_id: id,
                problem: addproblem
            }).then((res) => {
                showreportone()
                createproblemone2(id)
            })
    }

    const createproblemone2 = (id) => {
        if (rowsData2.length !== 0 && rowsData2 !== null) {
            for (const value of rowsData2) {
                const problemone = value.rowsData2
                try {
                    axios.post('http://localhost:3001/createproblemone', {
                        report_id: id,
            ...
            <Card>
                ...
                <div className="fit" style={{ marginLeft: '1.5em' }}>
                    <div className="grid p-fluid">
                        ...
                        <div className="col-12 md:col-3">
                            <InputText value={addquarterchargesone} onChange={(e) => setAddquarterchargesone(e.target.value)} style={{ width: '35em' }} placeholder="งบไตรมาสที่ 1" />
                        </div>
                    </div>
                </div>
                <div className="fit" style={{ marginLeft: '1.5em' }}>
                ...
                <div style={{ marginTop: '2em', marginLeft: '76em' }} >
                    <Button type="button" icon="pi pi-send" label='ส่ง' style={{ width: '7em' }} className="p-button-info" onClick={() => createquarterchargesone(location.state.project_id)} />
