...
const Dataleader = () => {
  ...
  const detailproject = (node) => {
    ...
    return <div>
      <Button type="button" icon="pi pi-eye" className="p-button-outlined p-button-secondary" onClick={() => history.push({ pathname: "/home/detailleader", state: node })
      }>
      </Button>
    </div>;
  }
  ...
  const report3 = (node, column) => {
    if (node.report_three === 0) {
      ...
      return <Button type="button" icon="pi pi-search" className="p-button-secondary" onClick={() => history.push({ pathname: "/homer/reportthree", state: node })} />
    }
  }
