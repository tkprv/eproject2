...
const Dataprojectevaluationmanager = () => {
  ...
  const manageproject = (node) => {
    if ((node.status === 6 || node.status === 7 || node.status == 8 || node.status === 9) && node.status_evaluation === 0) {
      return <div>
        ...
        <Tooltip placement="bottom" title={<span>อนุมัติเอกสารประเมินโครงการ</span>} ><Button type="button" icon="pi pi-pencil" className="p-button-warning" style={{ marginLeft: '.5em', height: '2.5em', width: '2.5em' }} onClick={() => history.push({ pathname: "/home/editprojectevaluationmanager", state: node })} /></Tooltip>
      </div>
    } else if ((node.status === 6 || node.status === 7 || node.status == 8 || node.status === 9) && (node.status_evaluation === 2 || node.status_evaluation === 3 || node.status_evaluation === 4)) {
      return <div>
        <Tooltip placement="bottom" title={<span>ดูเอกสารประเมินโครงการ</span>} ><Button type="button" icon="pi pi-search" className="p-button-info" style={{ height: '2.5em', width: '2.5em' }} onClick={() => history.push({ pathname: "/home/detailshowprojectevaluation", state: node })} /></Tooltip>
...
export default Dataprojectevaluationmanager